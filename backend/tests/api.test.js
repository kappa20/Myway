...
// Start the server immediately at module load, before any tests are registered.
// This avoids node:test's before() hook ordering issues in Node 18.
const server = http.createServer(app);
const baseUrlReady = new Promise((resolve) => {
  server.listen(0, () => {
...
async function fetchJson(path, options = {}) {
  const baseUrl = await baseUrlReady;
