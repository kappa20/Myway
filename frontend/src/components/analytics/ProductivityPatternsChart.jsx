    ...
    <div className="productivity-patterns">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <XAxis dataKey={xKey} stroke={getCSSVar('--text-secondary')} />
          <YAxis label={{ value: 'Sessions', angle: -90, position: 'insideLeft' }} stroke={getCSSVar('--text-secondary')} />
          <Tooltip
          ...
          />
          <Legend />
          <Bar dataKey="sessions" fill={getCSSVar('--primary-600')} name="Sessions" radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
